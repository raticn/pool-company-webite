...
export default {
    ...
    methods: {
        ...
        async astralPoolInfo($event) {
            ...
            try {
                let astral = await axios.get('http://091v123.mars2.mars-hosting.com/API/oprema', {params: {
    ...
    async mounted() {
        ...
        window.addEventListener("scroll", function () {
            let scrollHeight = this.window.pageYOffset
                if (scrollHeight > 70) {
                this.document.querySelector(".opremaToTop").style.display = "block"
            }
            else {
                this.document.querySelector(".opremaToTop").style.display = "none"
            }
        })
    },
    created() {
...
<div class="bazenskaOpremaWrapper" id="opremaTop">
    <div class="sideBar">
        <p id="sideBarSection1" class="activeSection" @click="sideBarToggle($event); astralPoolInfo($event)">Oprema za privatne bazene</p>
...
#sideBarSection1, #sideBarSection2{
    margin:1em 0 0 1em;
    cursor: pointer;
}
.activeSection{
